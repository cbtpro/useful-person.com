...
export default (props: IProps) => {
    ...
    const getSmsCode = async () => {
        setLoading(true)
        try {
            const { mobile, imageCode } = await sendSmsCodeForm.validateFields()
    ...
    const ok = async () => {
        setLoading(true)
        try {
            const { mobile, smsCode } = await bindNewMobileForm.validateFields()
            await updateMobile({ mobile, smsCode })
            props.onSuccess()
        } finally {
            setLoading(false)
        }
    ...
    return <>
        <Modal visible={props.visible} title="绑定手机号" onCancel={close} footer={[
            <Button key="cancel" onClick={close}>取消</Button>,
            <Button key="submit" loading={loading} onClick={ok}>确认</Button>
        ]}>
            <Form form={sendSmsCodeForm} {...layout}>
                <Form.Item label="手机号" name="mobile" rules={MOBILE_FORM_RULES}>
                    <Input onChange={e => bindNewMobileForm.setFieldsValue({ mobile: e.target.value})} />
                </Form.Item>
                <Form.Item label="验证码">
                    <Row gutter={8}>
                        <Col span={16}>
            ...
            <Form form={bindNewMobileForm} {...layout}>
                ...
                <Form.Item label="短信验证码">
                    <Row gutter={8}>
                        <Col span={16}>
                            <Form.Item name="smsCode" rules={[{ required: true, message: '短信验证码不能为空！' }, { pattern: /[0-9A-Za-z]{4,}/, message: '短信验证码只能是4位数字和字母，大小写忽略！' }]} noStyle >
                                <Input />
                            </Form.Item>
                        </Col>
                        <Col span={8}>
